...
app.get("/", (req: Request, res: Response) => {
  const messageObject = {
    message: "First Hello Word in the path '/', this is a victory for me",
  };
  res.send(messageObject);
});

app.get("/numbers", (req: Request, res: Response) => {
  const objectArray = [20, 30, 40, 50];
  res.send(objectArray);
});

app.get("/students", (req: Request, res: Response) => {
  if (!students) {
    res.status(404).send({ message: "Não existe estudantes!" });
  }
...
app.put('/products/:id',  (req: Request, res: Response) => {
 const productIndex = data.products.findIndex((prod) => prod.id === parseInt(req.params.id))
...
app.delete('/products/:id', (req: Request, res: Response) => {
  ...
  return res.send({message: 'Excluído com sucesso'})
})